...
class AuthController {
	...
	async login(req, res, next) {
		try {
			...
			if (!email || !password) return res.json({ failure: 'Email and password are required.' })

			const user = await userModel.findOne({ email, isDeleted: false })
			if (!user) return res.json({ failure: 'No account found with this email.' })

			if (!user.isVerified) {
				return res.json({ failure: 'Please verify your email before logging in.' })
			}

			const match = await bcrypt.compare(password, user.password)
			if (!match) return res.json({ failure: 'Incorrect password.' })
