...
import { registerUser, sendOtpEmail } from '@/actions/auth.action'
...
export default function RegisterPage() {
	...
	async function onSubmit(values: z.infer<typeof registerSchema>) {
		...
		try {
			...
			// Auto-verify the account (OTP is disabled)
			await sendOtpEmail({ email: values.email })

			toast.success(t('auth.register.success') || 'Account created! Please log in.')
			router.push('/login')
		} catch {
	...
	return (
		<div className='min-h-screen flex' style={{ background: 'linear-gradient(160deg,#0d0819 0%,#1a0a2e 100%)' }}>
			{/* Left panel */}
			<div className='hidden lg:flex flex-col justify-between w-96 shrink-0 p-10 border-r border-white/5'>
				...
				<div className='space-y-6'>
					<p className='text-3xl font-extrabold text-white leading-snug'>
						Start invoicing<br />
						<span style={{ color: '#FF2D78' }}>in minutes.</span>
					</p>
					<ul className='space-y-3'>
						{[
							'1 invoice free — no card needed',
							'Send to clients via email',
							'Upgrade anytime for more',
						].map(item => (
							<li key={item} className='flex items-center gap-2.5 text-sm text-white/70'>
								<div className='w-5 h-5 rounded-full factyo-gradient flex items-center justify-center shrink-0'>
			...
			<div className='flex-1 flex items-center justify-center p-6'>
				<div className='w-full max-w-sm space-y-8'>
					...
					<div>
						<h1 className='text-2xl font-extrabold text-white'>{t('auth.register.title')}</h1>
						<p className='text-white/50 text-sm mt-1'>
							{t('auth.register.subtitlePre')}{' '}
							<Link href='/login' className='font-semibold hover:underline' style={{ color: '#FF2D78' }}>
					...
					<Form {...form}>
						<form onSubmit={form.handleSubmit(onSubmit)} className='space-y-4'>
							<FormField
								...
								render={({ field }) => (
									<FormItem>
										<FormLabel className='text-white/70 text-sm'>{t('auth.register.name')}</FormLabel>
										<FormControl>
											<Input
							...
							<FormField
								...
								render={({ field }) => (
									<FormItem>
										<FormLabel className='text-white/70 text-sm'>{t('auth.register.email')}</FormLabel>
										<FormControl>
											<Input
							...
							<FormField
								...
								render={({ field }) => (
									<FormItem>
										<FormLabel className='text-white/70 text-sm'>{t('auth.register.password')}</FormLabel>
										<FormControl>
											<Input
							...
							<FormField
								...
								render={({ field }) => (
									<FormItem>
										<FormLabel className='text-white/70 text-sm'>{t('auth.register.confirmPassword')}</FormLabel>
										<FormControl>
											<Input
												placeholder='••••••••'
												type='password'
