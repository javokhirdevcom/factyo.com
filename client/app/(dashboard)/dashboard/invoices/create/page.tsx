...
import { Plus, Trash2, ChevronLeft, AlertCircle, UserPlus, ChevronDown } from 'lucide-react'
...
export default function CreateInvoicePage() {
	...
	const form = useForm<FormValues, unknown, FormValues>({
		...
		defaultValues: {
			clientId: '',
			invoiceNumber: `${new Date().getFullYear()}-${String(((session as any)?.currentUser?.invoiceSequence ?? 0) + 1).padStart(4, '0')}`,
			date: today,
			dueDate: due,
	...
	async function onSubmit(values: FormValues) {
		...
		setLoading(true)
		const userId = user?._id
		const res = await createInvoiceAction({ userId, ...values, subtotal, vatAmount, total })
		if (res?.data?.limitReached) {
			setLimitError(res.data.failure ?? t('invoices.limitReachedFree'))
			setLoading(false)
			return
		}
		if (res?.data?.failure) {
		...
		if (res?.data?.invoice) {
			...
			router.push(`/dashboard/invoices/${res.data.invoice._id}`)
		}
		setLoading(false)
	}
	...
	return (
		<div className='max-w-3xl mx-auto space-y-6'>
			<div className='flex items-center gap-3'>
				...
				<div>
					<h1 className='text-2xl font-extrabold text-gray-900'>{t('invoices.createInvoice')}</h1>
					<p className='text-gray-500 text-sm'>Fill in the details below and send or save as draft.</p>
				</div>
			</div>
			...
			<Form {...form}>
				<form onSubmit={form.handleSubmit(onSubmit)} className='space-y-6'>
					...
					<div className='flex gap-3 justify-end'>
						<Link href='/dashboard/invoices' className='px-5 py-2.5 rounded-xl border border-gray-200 text-sm font-semibold text-gray-600 hover:bg-gray-50 transition-colors'>
